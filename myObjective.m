...
function muscleActivation = findMuscleActivation(input)
    A1 = input(1);  
    A2 = input(2);
    A3 = input(3);
    A4 = input(4);
...
% 
    ...
    %dataMatrix
    for trial = 3:5 %should be 1:3
        for activity = 3:3 %should be 1:7
            ...
            theta_estimates = zeros(4000, 69);  %use gpr to find this, compute/estimate from EMG signal data
...
function f = leastSquares(input)
            ...
            for timestep = 1:4000
                ...
                [predicted_mean, predicted_var] = gp(hyp2, @infGaussLik, meanfunc, covfunc, likfunc, v(t), theta_targets(t, :));
...
function f_vector = testing(input)
     % input is the optimized values for each variable
    A1 = input(1);    
    A2 = input(2);
    A3 = input(3);
    A4 = input(4);
    ...
    for trial = 1:2 %should be 4:5
        for activity = 3:3 %should be 1:7
            ...
            for dof = 1:69
                ...
                for i = 1:4000
                    f = f + (theta_estimates(i, dof)-theta_targets(i, dof))^2;
                    theta_targets_sum = theta_targets_sum + theta_targets(i,dof);
                    theta_estimates_sum = theta_estimates_sum + theta_estimates(i, dof);
                end
                                   
            end
            theta_estimates
            save('t1a1_estimates.mat', 'theta_estimates');
            
            theta_targets 
            nrmse = sqrt((((theta_targets_sum - theta_estimates_sum)/theta_targets_sum)^2)/N);
            
            f_vector(index, 1) = nrmse;
            index = index + 1;
        end
    end
end
